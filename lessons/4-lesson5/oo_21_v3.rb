...
class Player
  ...
  def total
    ...
    if pre_sum > 21 && large_ace_count >= 1
  ...
  def show_cards
    puts "\n#{self.class}'s cards are: "
    cards_in_hand.each { |card| puts card }
  end

  def busted?
    puts "#{self.class.name.capitalize} busted!".center(80) if total > 21
    total > 21
  end
  ...
  def readjust_cards_value
    until sum_of_cards_value <= 21 || large_ace_count == 0
      degrade_an_ace
    end
  end
...
class Game
  ...
  def human_turn
    human.make_choice
    deal_to(human) if human.current_move == 'hit'
    clear_screen
  ...
  def npc_turn
    npc.make_choice
    deal_to(npc) if npc.current_move == 'hit'
